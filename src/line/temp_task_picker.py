...
async def list_pending_tasks(session: AsyncSession, user_id: UUID) -> list[PendingTask]:
    ...
    time answering a form Go can't save yet (see
    src/line/router.py's _UNSUPPORTED_HANDLERS check) -- previously this
    picker had no way to know that at all, and the farmer only found out
    after answering every question and tapping confirm.
    """
