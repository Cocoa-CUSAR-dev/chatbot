...
async def _run(*, due, owing, done, id_map, multicast_batched=None):
    ...
    with (
        ...
        patch("src.reminders.jobs.deactivate_schedule", new=AsyncMock()) as deactivate,
    ):
        await _run_reminder_check(session, _NOW)
    return record, multicast_batched, deactivate
...
async def test_pushes_to_users_who_still_owe_and_logs_them():
    a, b = uuid4(), uuid4()
    tid = uuid4()
    record, multicast_batched, _deactivate = await _run(
...
async def test_users_already_reminded_today_are_skipped():
    a, b = uuid4(), uuid4()
    tid = uuid4()
    record, _, _deactivate = await _run(due=_due(tid), owing=[a, b], done={a}, id_map={b: "U2"})
    assert record.await_args.kwargs["user_ids"] == [b]


async def test_no_due_schedules_is_a_no_op():
    record, multicast_batched, _deactivate = await _run(due=[], owing=[], done=set(), id_map={})
...
async def test_everyone_owing_already_reminded_sends_nothing():
    ...
    tid = uuid4()
    record, multicast_batched, _deactivate = await _run(
        due=_due(tid), owing=[a], done={a}, id_map={}
    )
    multicast_batched.assert_not_awaited()
    record.assert_not_awaited()


async def test_nobody_owes_the_task_deactivates_the_schedule():
    """web-app UI's "farmer completed the task" auto-off: once
    users_owing_task comes back empty, there's no one left to remind for
    this task -- turn the schedule off instead of checking it forever.
    """
    tid = uuid4()
    schedule = _due(tid)
    record, multicast_batched, deactivate = await _run(
        due=schedule, owing=[], done=set(), id_map={}
    )
    multicast_batched.assert_not_awaited()
    record.assert_not_awaited()
    deactivate.assert_awaited_once_with(ANY, schedule[0].schedule_id)


async def test_deactivation_does_not_fire_while_someone_still_owes_the_task():
    a = uuid4()
    tid = uuid4()
    _record, _multicast_batched, deactivate = await _run(
        due=_due(tid), owing=[a], done=set(), id_map={a: "U1"}
    )
...
async def test_line_failure_still_logs_but_with_failed_status():
    ...
    record, _, _deactivate = await _run(
